...
export const CustomThemeProvider: FC<Props> = ({ children }) => {
  ...
  useEffect(() => {
    const storageTheme = localStorage.getItem("footerActiveTheme");
    const isSavedTheme =
      storageTheme !== null && (storageTheme === "light" || storageTheme === "dark");
    setActiveTheme(isSavedTheme ? getActiveTheme(storageTheme) : darkTheme);
  }, []);

  useEffect(() => {
    if (activeTheme) {
      localStorage.setItem("footerActiveTheme", activeTheme.palette.mode);
    }
  }, [activeTheme]);
