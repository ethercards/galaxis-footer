import { ContactUsRequest } from '../galaxis-footer/contact-us-form/contact-us-form.component';
...
export const isChanged = <T extends ContactUsRequest>(
  ...
  return Object.keys(currentObject).some(
    (field) =>
      currentObject[field as keyof T] !== originalObject[field as keyof T],
  );
};
...
export const isValid = <T extends ContactUsRequest>(
  ...
  const isValid = mandatoryFields.every(
    (key) => !!currentObject[key as keyof T],
  );
